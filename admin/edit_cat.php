...
<body class="bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-white min-h-screen flex transition-colors">
    ...
    <!-- Main Content -->
    <main class="ml-0 lg:ml-72 flex-1 p-6 lg:p-12 mt-20 lg:mt-0 max-w-4xl">
        <header class="mb-8">
            ...
            <h1 class="text-3xl lg:text-4xl font-black mb-2">Edit Cat</h1>
            <p class="text-slate-500 dark:text-slate-400">Update details for <?php echo htmlspecialchars($cat['name']); ?>.</p>
        </header>

        <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 lg:p-10 shadow-sm border border-slate-100 dark:border-slate-700">
            <form id="editCatForm" onsubmit="submitCat(event)">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <label class="flex-1 aspect-square rounded-[2rem] border-3 border-dashed border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden group">
                            ...
                            <div id="uploadPlaceholder" class="text-center p-6 <?php echo $cat['master_photo'] ? 'hidden' : ''; ?>">
                                <i class="fas fa-camera text-4xl text-slate-300 mb-2"></i>
                                <p class="text-sm font-bold text-slate-400">Click to change photo</p>
                            </div>
                            <img id="imagePreview" src="../<?php echo $cat['master_photo']; ?>" class="<?php echo $cat['master_photo'] ? '' : 'hidden'; ?> absolute inset-0 w-full h-full object-cover">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" value="<?php echo htmlspecialchars($cat['name']); ?>" required class="w-full h-14 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold focus:ring-2 focus:ring-amber-500">
                        </div>
                        
                        <div>
                            <label class="block font-bold text-slate-700 dark:text-slate-300 mb-2">Location</label>
                            <input type="text" name="location" value="<?php echo htmlspecialchars($cat['location']); ?>" required class="w-full h-14 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold focus:ring-2 focus:ring-amber-500">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="rarity" class="w-full h-14 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold focus:ring-2 focus:ring-amber-500">
                                <option value="common" <?php echo $cat['rarity'] == 'common' ? 'selected' : ''; ?>>Common</option>
                                <option value="rare" <?php echo $cat['rarity'] == 'rare' ? 'selected' : ''; ?>>Rare</option>
                                <option value="legendary" <?php echo $cat['rarity'] == 'legendary' ? 'selected' : ''; ?>>Legendary</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <textarea name="description" rows="4" required class="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-medium focus:ring-2 focus:ring-amber-500"><?php echo htmlspecialchars($cat['description']); ?></textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        <label class="block font-bold text-slate-700 dark:text-slate-300 mb-2">Likes</label>
                        <input type="text" name="likes" value="<?php echo htmlspecialchars($cat['likes']); ?>" class="w-full h-14 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-medium focus:ring-2 focus:ring-green-500">
                    </div>
                    <div>
                        <label class="block font-bold text-slate-700 dark:text-slate-300 mb-2">Dislikes</label>
                        <input type="text" name="dislikes" value="<?php echo htmlspecialchars($cat['dislikes']); ?>" class="w-full h-14 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-medium focus:ring-2 focus:ring-red-500">
                    </div>
                </div>
                ...
                <button type="submit" id="submitBtn" class="w-full py-5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-black text-xl shadow-xl shadow-amber-500/20 transition-all active:scale-95 flex items-center justify-center gap-2">
                    <i class="fas fa-save"></i> Update Cat Profile
                </button>
            </form>
        </div>
    ...
    <script>
        ...
        function submitCat(e) {
            ...
            formData.append('action', 'edit_cat');
